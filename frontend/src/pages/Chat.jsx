...
function colorFor(name) {
    // Foydalanuvchi nomidan barqaror rang
    const colors = [
...
export default function Chat() {
    ...
    useEffect(() => {
        ...
        load();
        const id = setInterval(load, 4000); // har 4 soniyada yangilash
    ...
    async function send(e) {
        ...
        setText("");
        try {
            await api.post("/api/chat/messages/", { text: value });
            await load();
        } catch {
            setText(value); // xato bo'lsa matnni qaytaramiz
        } finally {
            setSending(false);
        }
    }
            ...
            <div className="th-card flex flex-col" style={{ height: "70vh" }}>
                                    ...
                                    <div
                                        className={`max-w-[75%] rounded-2xl px-4 py-2 ${
                                            mine
                                                ...
                                                : "bg-gray-100 dark:bg-slate-700 rounded-bl-sm"
                                        }`}
                                    >
                                        {!mine && (
                                            <div className="text-xs font-bold text-indigo-500 mb-0.5">
                                                {m.username}
                                            </div>
                                        )}
                ...
                <form onSubmit={send} className="mt-4 flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="th-btn-blue px-4 py-2"
                    >
                        <Send size={18} />
